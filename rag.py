...
def process_question_with_reasoning(question: str, kb_name: str = DEFAULT_KB, use_search: bool = True, use_table_format: bool = False, multi_hop: bool = False, chat_history: List = None):
    ...
    try:
        ...
        yield f"### 错误信息\n{error_msg}", str(f"处理您的问题时遇到错误：{str(e)}")
...
with gr.Blocks(title="Hop-RAG 医疗知识问答系统", elem_id="app-container") as demo:
    ...
    # 清空对话历史按钮功能
    def clear_history():
        # 返回空列表给chatbot和chat_history_state（Gradio格式）
        return [], []
    ...
    def convert_to_gradio_format(chat_history):
        """将旧格式的聊天历史转换为Gradio 6.0格式"""
        if not chat_history:
            return []
        ...
        for msg_pair in chat_history:
            if isinstance(msg_pair, list) and len(msg_pair) == 2:
                ...
                # 添加用户消息
                gradio_format.append(ChatMessage(role="user", content=str(user_msg)))
                # 添加助手消息
                gradio_format.append(ChatMessage(role="assistant", content=str(assistant_msg)))
            elif isinstance(msg_pair, dict) and 'role' in msg_pair and 'content' in msg_pair:
                # 已经是Gradio格式
                gradio_format.append(msg_pair)
            elif isinstance(msg_pair, ChatMessage):
                # 已经是ChatMessage对象
                gradio_format.append(msg_pair)

        return gradio_format

    def add_message_pair(chat_history, user_msg, assistant_msg):
        """添加一对消息到聊天历史（Gradio 6.0格式）"""
        if chat_history is None:
            chat_history = []

        # 添加用户消息
        chat_history.append(ChatMessage(role="user", content=str(user_msg)))
        # 添加助手消息
        chat_history.append(ChatMessage(role="assistant", content=str(assistant_msg)))

        return chat_history

    def update_last_assistant_message(chat_history, new_content):
        """更新最后一条助手消息的内容"""
        if not chat_history:
            return chat_history
    ...
    def process_and_update_chat(question, kb_name, use_search, use_table_format, multi_hop, chat_history):
        if not question.strip():
            # 确保返回Gradio格式
            if chat_history and not isinstance(chat_history[0], (dict, ChatMessage)):
                chat_history = convert_to_gradio_format(chat_history)
        ...
        try:
            ...
            # 如果chat_history是旧格式，转换为Gradio格式
            if chat_history and not isinstance(chat_history[0], (dict, ChatMessage)):
                chat_history = convert_to_gradio_format(chat_history)
            ...
            # 添加用户消息和初始助手消息
            chat_history = add_message_pair(chat_history, question_str, "正在思考...")
            ...
            # 使用生成器进行流式处理
            # 注意：process_question_with_reasoning期望旧格式的chat_history
            # 所以我们需要传递旧格式的历史
            old_format_history = []
            for i in range(0, len(chat_history) - 2, 2):  # 排除最后添加的一对消息
                if i + 1 < len(chat_history):
                    user_msg = chat_history[i].content if isinstance(chat_history[i], ChatMessage) else chat_history[i].get('content', '')
                    assistant_msg = chat_history[i + 1].content if isinstance(chat_history[i + 1], ChatMessage) else chat_history[i + 1].get('content', '')
                    old_format_history.append([user_msg, assistant_msg])

            for search_display, answer in process_question_with_reasoning(question_str, kb_name, use_search, use_table_format, multi_hop, old_format_history):
                # 更新检索状态和答案
                last_search_display = search_display
                last_answer = str(answer) if answer is not None else "未生成回答"

                # 更新聊天历史中的最后一条助手消息
                chat_history = update_last_assistant_message(chat_history, last_answer)
            ...
            chat_history = update_last_assistant_message(chat_history, str(error_msg))
...
if __name__ == "__main__":
    demo.launch(server_name="127.0.0.1", server_port=7860, share=True, theme=custom_theme, css=custom_css)